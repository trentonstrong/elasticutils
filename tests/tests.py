...
from elasticutils import F, Q, get_es
...
class QueryTest(TestCase):
    ...
    def test_filter_and(self):
        eq_(len(Q(tag='awesome', foo='bar')), 1)

    def test_filter_or(self):
        eq_(len(Q().filter(F(tag='awesome')|F(tag='boat'))), 4)

    def test_filter_or_3(self):
        eq_(len(Q().filter(F(tag='awesome')|F(tag='boat')|F(tag='boring'))), 5)

    def test_filter_complicated(self):
        eq_(len(Q().filter(F(tag='awesome', foo='bar')|F(tag='boring'))), 2)
